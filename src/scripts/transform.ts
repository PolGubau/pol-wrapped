...
export const processJsonData = async (): Promise<void> => {
  try {
    ...
    const data = await promises.readFile(OUTPUT_PATH, "utf-8");
    ...
    if (fileExists) {
      await promises.unlink(OUTPUT_PATH);
      console.info("%c🟡 Archivo JSON antiguo eliminado.", "color: yellow");
    }

    // Sobrescribir el archivo transformado
    await promises.writeFile(OUTPUT_PATH, JSON.stringify(transformedData, null, 2));
    console.info(`%c🟢 Archivo JSON transformado y sobrescrito en: ${OUTPUT_PATH}`, "color: green");
  } catch (error) {
